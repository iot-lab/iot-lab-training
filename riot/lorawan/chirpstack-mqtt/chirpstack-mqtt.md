...
**Prerequisites:** You must have followed at least the [getting started ChirpStack notebook](../chirpstack-getting-started/chirpstack-getting-started.md) before starting this one.
...
In this notebook, you will continue the work done in the [getting started ChirpStack notebook](../chirpstack-getting-started/chirpstack-getting-started.md) and use the MQTT protocol to be able to receive locally the sensor values measured by the LoRa end-device. You will also adapt the RIOT application to be able to receive messages sent via MQTT.
...
The ChirpStack API defines MQTT topics to interact with the end-devices (replace `<AppId>` with your application id):
- subscribe to the topic **application/<AppId>/device/+/event/up** to receive messages sent by the end-device.

### Use MQTT clients to receive messages in ChirpStack
...
Next, there are some steps to join the MQTT to your application:

1. You will integrate your application to the MQTT forwarder available in the ChirpStack console;
2. ChirpStack uses the TLS authentication mode. Once you select the MQTT integration, you will generate the certificates to authenticate to the MQTT service in order to transmit and receive data.
3. Digital signatures will be generated in order to sign and authenticate the connection to the MQTT broker.Into **certs folder** you can _copy_ -> _paste_ the credentials for each type of certificate/key inside **-----BEGIN CERTIFICATE-----** and **-----END CERTIFICATE-----**.

Follow the MQTT integration and the certificates generation in the next step-by-step video:

```python
from IPython.display import Video
Video("images/Video4-0.5x.mp4", embed=True, width=500)
```
...
6. Open another terminal with `File > New > Terminal` and run the following command there (we explain below what to use for `<AppId>`):
...
### Send messages to the end-device
...
To send messages with MQTT to the end device, the final step is use the ChirpStack to send messages to the end-device. For that, it is needed to generate downlink queue-items in the ChirpStack database. One a receive window occurs (triggered by an uplink), ChirpStack will transmit the items in the downlink queue FIFO to the device.

You can configure the messages to be sent to the end-device. In the ChirpStack console an uplink message can be defined:

<figure style="text-align:center">
    <img src="images/mqtt-downlink.png" width="500" align="center"/><br/><br/>
    <figcaption><em>Enqueuing downlink messages in the ChirpStack console<br/></em></figcaption>
</figure>
